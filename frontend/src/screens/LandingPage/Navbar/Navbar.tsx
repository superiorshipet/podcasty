...
import { useNavigate } from "react-router-dom"; // <-- (1) التعديل الأول

export const Navbar = (): JSX.Element => {
  const navigate = useNavigate(); // <-- (2) التعديل الثاني
      ...
      <div className="flex h-9 items-center justify-between pr-[-4.58e-05px] pl-0 py-0 relative self-stretch w-full">
        ...
        <div className="flex w-[167.09px] h-9 items-center gap-4 relative">
          <button className="all-[unset] box-border flex flex-col w-[69.08px] h-9 items-start relative">
            {/* ملاحظة: الكود الأصلي من Anima يحتوي على زر بداخل زر، وهذا غير صحيح في HTML.
              لقد أضفت الـ onClick على الزر الداخلي ليعمل.
            */}
            <button
              onClick={() => navigate("/login")} // <-- (3) التعديل الثالث
              className="all-[unset] box-border self-stretch w-full bg-white border-[0.8px] border-solid border-[#0000001a] h-9 relative rounded-lg"
            >
